...
select customer.customer_id, customer.first_name, customer.last_name, round(sum(invoice.total), 2) as total_spent
from customer
...
--- Q7.Let us invite the artists who have written the most rock music in our dataset. Write a query that returns the
...
--- that returns the country along with the top customer and howmuch they spent. For countries where the top amount 
...
(
 select customer.customer_id, first_name, last_name, billing_country, round(sum(total),2) as total_spent,
       dense_rank() over(partition by billing_country order by sum(total) desc) as denserank
 from invoice
 join customer on customer.customer_id = invoice.customer_id
 group by 1,2,3,4
 order by 4 asc, 5 desc
)
 select * from customer_with_country where denserank <=1;
